...
export const AuthProvider = ({ children }) => {
  ...
  // Update watched status (authenticated users only)
  const updateWatchedStatus = async (movieId, watched) => {
    if (!user) {
      throw new Error('Must be logged in to update watched status')
    }
  ...
  const removeFromWatchlist = async (movieId) => {
    try {
      if (user) {
        // Remove from authenticated user's watchlist
        // First get the like_id
        const { data: likedMovie, error: findError } = await supabase
          .from('liked_movies')
          .select('id')
          .eq('user_id', user.id)
          .eq('movie_id', movieId)
        ...
        const { error: sourcesError } = await supabase
          .from('liked_movie_sources')
          .delete()
          .eq('like_id', likedMovie.id)

        if (sourcesError) throw sourcesError

        // Then delete the liked movie
        ...
        // Remove from anonymous user's watchlist
        const currentSessionId = sessionId || localStorage.getItem('celluloid_session_id')
